export const impactStats = [
	{
		value: "November 13, 2025",
		label:
			"Twelve people (ages 15 to 56) graduated from our first ALC computer skills class",
		icon: "calendar",
	},
	{
		value: "December 26, 2025",
		label: "Eight people graduated from the first Launching Leaders class",
		icon: "calendar",
	},
	{
		value: "December 26, 2025",
		label: "Launching Leaders first graduation",
		icon: "calendar",
	},
	{
		value: "Ongoing",
		label: "Graduate mentoring now active",
		icon: "messageSquare",
	},
] as const;

export const impactSellingCards = [
	{
		title: "For learners: skills plus self-discipline",
		detail:
			"ALC combines practical digital training with growth mindset, time management, and confidence development.",
	},
	{
		title: "For donors: direct mission alignment",
		detail:
			"Support goes to instructor capacity, laptop access, and practical classroom delivery in underserved communities.",
	},
	{
		title: "For communities: local capability building",
		detail:
			"Programs and mentoring strengthen youth readiness for education, enterprise, and community contribution.",
	},
] as const;

export const impactProofPoints = [
	"Graduates completed a structured leadership curriculum with practical self-improvement tools",
	"Remote Micronesiaese-language instruction enabled access despite geographic isolation",
	"Post-graduation mentoring now supports long-term follow-through and goal attainment",
] as const;

export const impactDonorProofBlocks = [
	{
		metric: "Every month",
		title: "Recurring support keeps classes predictable",
		detail:
			"Monthly gifts stabilize materials, power/connectivity, and laptop upkeep for uninterrupted learning.",
	},
	{
		metric: "Direct",
		title: "Gifts convert into classroom outcomes",
		detail:
			"Donor funds move straight into learner access, practical instruction time, and functioning equipment.",
	},
	{
		metric: "Visible",
		title: "Impact can be seen in real learner progress",
		detail:
			"Story and testimonial evidence reflects actual skills gained, not abstract promises.",
	},
] as const;

export const impactAsks = [
	"Help pay for instructors where local teaching capacity is limited",
	"Fund laptops so additional classrooms can open in Micronesia",
	"Support expansion of mentoring and leadership cohorts",
] as const;
